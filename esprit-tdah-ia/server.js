...
`;
  ...
  // À partir d'ici : appel OpenAI EN STREAMING et renvoi token par token
  try {
    ...
    log(
      "Calling OpenAI (streaming) with model:",
      openAiModel,
      "| usedSearch:",
      usedSearch,
      "| volatileFinal:",
    ...
    // Préparer la réponse HTTP en mode streaming texte
    ...
    if (!openaiResp.ok) {
      ...
      log("OpenAI error (stream):", openaiResp.status, t);
      res.write("Désolé, une erreur est survenue côté IA.");
      res.end();
      return;
    }
    ...
    while (true) {
      ...
      buffer = lines.pop(); // garde la dernière éventuelle ligne incomplète
    ...
    // Fin du stream vers le client
    res.end();

    // Mise à jour de la mémoire après avoir reconstruit la réponse complète
    const answer = fullAnswer || "Désolé, je n'ai pas pu générer de réponse.";
    ...
    // On ne renvoie plus de JSON ici, tout est déjà parti en streaming
    log("Réponse streaming terminée, longueur:", answer.length);
  } catch (e) {
    log("Erreur serveur (streaming /chat):", e);
    try {
      if (!res.headersSent) {
